...
#define PORT 8080
#define BUFFER_SIZE 1024 // Дескриптор соединения c MySql
MYSQL mysql;
MYSQL_RES* res;
MYSQL_ROW row;
SOCKET clientSocket;
...
void showListOfUsers(SOCKET clientSocket)//вывод списка пользователей
{
    response_to_client = "";
    string tmp;
    mysql_query(&mysql, "SELECT login FROM users_list"); //Делаем запрос к таблице
    if (res = mysql_store_result(&mysql)) {
...
void printPublicChatMes(SOCKET clientSocket)//вывод общего чата
{
    response_to_client = "";
    string tmp;
    mysql_query(&mysql, "SELECT * FROM all_chat"); //Делаем запрос к таблице
    if (res = mysql_store_result(&mysql)) {
        if (mysql_num_rows(res) == 0) {
...
void personalMess(string& mes, SOCKET clientSocket) {//отправка личного сообщения
    ...
    mysql_free_result(res);
    mysql_query(&mysql, to_query.c_str()); //Делаем запрос к таблице
    res = mysql_store_result(&mysql);
    if (mysql_num_rows(res) > 0) {
        row = mysql_fetch_row(res);
        temp = row[0];
